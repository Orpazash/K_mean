...
int add_to_cluster(vector *vec, cluster *curr_cluster)  /*adds vector to the right cluster*/
{
    ...
    cluster_item *cluster_item = NULL;
...
int main(int argc, char **argv) {
    int D = 0, N, i, K, iter = 200, iter_count = 0;
    cluster head_cluster, *curr_cluster, *old_cluster;
    ...
    curr_vec = head_vec;
    curr_cluster = &head_cluster;  /*curr cluster is first cluster*/
    curr_cluster->first_item = NULL;

    for(i = 0; i < K; ++i ) {
        cord new_cord, *cur_cord, *new_curr_cord;
        ...
        cur_cord = curr_vec->cords;
        new_curr_cord= &new_cord;
    ...
    old_cluster = head_cluster.next; /*naming first cluster as 'old cluster', start from sentinel's next*/

    while(1) {
        int ret;
        cluster new_clusters, *curr_old, *new_cluster, *temp;
        /*check if we finished the iterations*/
        if(iter_count == iter)
            break;

        /* assign every dot to the closest cluster*/
        ret = add_all_to_clusters(head_vec , old_cluster);
        if (ret == 0) {
